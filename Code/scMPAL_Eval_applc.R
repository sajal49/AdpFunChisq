...
# Subset data for Viral Carcinogenesis pathway
Prepare_ViralCarcinogen_Data = function()
{
  ...
  surface_markers = c("CD14", "CD3D", "CD19", "CD8A")
  surface_proteins = norm_proteins[match(surface_markers, alt_protein_names),]
  
  # get genes involved in Viral Carcinogen
  vcgenes = keggGet(dbentries = "hsa05203")
  vcgenes = vcgenes[[1]]$GENE[c(FALSE,TRUE)]
  vcgenes[76] = "H2BS1"
  vcgenes = gsub(";.*","",vcgenes)
  
  # find genes in scRNA
  pthwGenes = match(vcgenes, gene_names)
  rnaGenes = norm_scdata[pthwGenes[!is.na(pthwGenes)],]
  pthwGenes = gene_names[pthwGenes[!is.na(pthwGenes)]]
  
  # save subset under Data/MPAL/
  rm(list = setdiff(ls(), c("pthwGenes", "surface_proteins", "surface_markers", "rnaGenes")))
  save.image("../Data/MPAL/sub_data_for_viral_carcinogens_4_surface_protein.RData")
}
...
MPAL_ViralCarcinogen_Interactions_FVNF = function(){
  ...
  results = pblapply(c(1:length(parent_index)), function(i){
    ...
    if(length(p) < 10){
      ...
      # Discretize jointly using GridOnCluster
      tbl = discretize.jointly(cbind(p,c), k = k)  
...
PlotTopViralCarcinogenPatterns = function(scores){
  
  load("../Data/MPAL/sub_data_for_viral_carcinogens_4_surface_protein.RData")
  
  # each protein gets paired with 164 RNAs
  parent_index = rep(c(1:4), each=nrow(rnaGenes))
  child_index = rep(c(1:nrow(rnaGenes)), 4)
  ...
  for(i in 1:length(methodnames)){
    ...
    for(j in 1:5){
      ...
      png(paste0("../Results/MpalStudy/TopPatterns/Pathway/",methodnames[i],"_",j,".png"))
...
MPAL_study = function(){
  ...
  dev.off()
  save(known_eval_fvnf_scores, file="../Results/MpalStudy/known_eval_fvnf_scores.RData")
  ...
  dev.off()
  save(known_eval_fvi_scores, file="../Results/MpalStudy/known_eval_fvi_scores.RData")
  ...
  # prepare a subset of data for regulatory control of viral carcinogenesis pathway
  if(!file.exists("../Data/MPAL/sub_data_for_viral_carcinogens_4_surface_protein.RData")){
    Prepare_ViralCarcinogen_Data()
  }
  
  # find pairs in Viral carcinogens pathway
  pathway_fvnf_scores = MPAL_ViralCarcinogen_Interactions_FVNF()
  save(pathway_fvnf_scores, file="../Results/MpalStudy/pathway_fvnf_scores.RData")
