...
# Place abalone.data in Data/Abalone/ directory if not already available.
...
abalone_fvnf_test = function(){
  ...
  for(i in 1:length(sm)){
    ...
    for(r in 1:repeats){
      ...
      d = discretize.jointly(data = log1p(apply(abalone_s[,-1], c(1,2), as.numeric)), 
                             k=Inf,
...
DiscretizationLvlCheck = function(){
  ...
  # discretize data jointly (Length, Diameter and Height are discretized together)
  d = discretize.jointly(data = log1p(apply(abalone[,-1], c(1,2), as.numeric)), 
                         k=Inf,
                         cluster_method = 'kmeans+silhouette',
                         min_level = 2,
                         grid_method = 'Sort+split')
  
  results = as.numeric(apply(d$D, 2, max))
  
  cat("Minimum Discretization Levels: ", 
      min(results), "\n")
  cat("Mean Discretization Levels: ",
      mean(results),"\n")
  cat("Maximum Discretization Levels: ", 
      max(results), "\n")
  
  # construct results list
  # 'Sex' has 3 levels, integrating number of discretization levels for other
  # variables
  aba_results = vector("list", length(results))
  for(i in 1:length(results)){
    aba_results[[i]] = c(3, results[i])
  }
  
  # table type stats
  ttype = rep("", length(aba_results))
  for(i in 1:length(aba_results)){
    if(aba_results[[i]][1] == aba_results[[i]][2]){
      ttype[i] = "square"
    } else if(aba_results[[i]][1] > aba_results[[i]][2]){
      ttype[i] = "portrait"
    } else {
      ttype[i] = "landscape"
    }
  ...
  abalone_table_types = ttype
  tab_ttype = table(abalone_table_types)
  ...
  save(aba_results,file="../Data/Abalone/Table_Stats.RData")
...
AbaloneEval = function(){
  ...
  save(abalone_fvnf, file="../Data/Abalone/abalone_fvnf.Rds")
  ...
  save(abalone_fvi, file="../Data/Abalone/abalone_fvi.Rds")
